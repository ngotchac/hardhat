...
const HardhatNetworkMempoolConfig = t.type({
    order: io_ts_1.optional(t.keyof(lang_1.fromEntries(constants_1.HARDHAT_MEMPOOL_SUPPORTED_ORDERS.map((order) => [order, null])))),
});
const HardhatNetworkMiningConfig = t.type({
    auto: io_ts_1.optional(t.boolean),
    interval: io_ts_1.optional(t.union([t.number, t.tuple([t.number, t.number])])),
    mempool: io_ts_1.optional(HardhatNetworkMempoolConfig),
});
