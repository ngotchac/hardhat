...
class HardhatNode extends events_1.default {
    constructor(_vm, _stateManager, _blockchain, _txPool, _automine, _minGasPrice, _blockTimeOffsetSeconds = new ethereumjs_util_1.BN(0), _mempoolOrder, genesisAccounts, tracingConfig, _forkNetworkId, nextBlockBaseFee) {
        super();
        this._vm = _vm;
    ...
    static async create(config) {
        ...
        const node = new HardhatNode(vm, stateManager, blockchain, txPool, automine, minGasPrice, initialBlockTimeOffset, mempoolOrder, genesisAccounts, tracingConfig, forkNetworkId, nextBlockBaseFeePerGas);
        return [common, node];
    }
...
Hardhat Network's forking functionality only works with blocks from at least spuriousDragon.`);
    ...
    async mineBlock(shouldEmitEvents, timestamp) {
        ...
        await this._saveBlockAsSuccessfullyRun(result.block, result.blockResult, shouldEmitEvents);
        if (needsTimestampIncrease) {
     ...
     */
    async _saveBlockAsSuccessfullyRun(block, runBlockResult, shouldEmitEvents) {
