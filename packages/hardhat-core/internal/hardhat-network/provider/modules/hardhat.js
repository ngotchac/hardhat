...
class HardhatModule {
    ...
    async _intervalMineAction() {
        const result = await this._node.mineBlock(true);
    ...
    async _setMinGasPriceAction(minGasPrice) {
        ...
        if (this._node.isEip1559Active()) {
            throw new errors_1.InvalidInputError("hardhat_setMinGasPrice is not support when EIP-1559 is active");
        }
    ...
    _setBalanceParams(params) {
        return validation_1.validateParams(params, base_types_1.rpcAddress, base_types_1.rpcQuantity);
    }
    ...
    _setStorageAtParams(params) {
        const [address, positionIndex, value] = validation_1.validateParams(params, base_types_1.rpcAddress, base_types_1.rpcQuantity, base_types_1.rpcData);
        ...
        if (value.length !== 32) {
            throw new errors_1.InvalidInputError(`Storage value must be exactly 32 bytes long. Received ${base_types_1.bufferToRpcData(value)}, which is ${value.length} bytes long.`);
        }
