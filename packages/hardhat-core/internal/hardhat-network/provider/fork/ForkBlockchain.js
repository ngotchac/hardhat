...
class ForkBlockchain {
    ...
    async _processRemoteBlock(rpcBlock) {
        ...
        for (const transaction of rpcBlock.transactions) {
            ...
            if (transaction.type === undefined || transaction.type.eqn(0)) {
                tx = new ReadOnlyValidTransaction_1.ReadOnlyValidTransaction(new ethereumjs_util_1.Address(transaction.from), rpcToTxData_1.rpcToTxData(transaction));
            }
            else if (transaction.type.eqn(1)) {
                tx = new ReadOnlyValidEIP2930Transaction_1.ReadOnlyValidEIP2930Transaction(new ethereumjs_util_1.Address(transaction.from), rpcToTxData_1.rpcToTxData(transaction));
            }
            else if (transaction.type.eqn(2)) {
                tx = new ReadOnlyValidEIP1559Transaction_1.ReadOnlyValidEIP1559Transaction(new ethereumjs_util_1.Address(transaction.from), rpcToTxData_1.rpcToTxData(transaction));
            }
            else {
    ...
    _processRemoteTransaction(rpcTransaction) {
        ...
        const transaction = new ReadOnlyValidTransaction_1.ReadOnlyValidTransaction(new ethereumjs_util_1.Address(rpcTransaction.from), rpcToTxData_1.rpcToTxData(rpcTransaction));
    ...
    async _processRemoteReceipt(txReceipt) {
        ...
        const receipt = output_1.remoteReceiptToRpcReceiptOutput(txReceipt, tx, output_1.shouldShowTransactionTypeForHardfork(this._common), output_1.shouldShowEffectiveGasPriceForHardfork(this._common));
        this._data.addTransactionReceipt(receipt);
        return receipt;
    }
}
